...
class StockEntry(StockController):
	...
	def remove_items(self):
		if self.stock_entry_type=="Material Transfer for Virtual":
			parent_doc = frappe.get_doc("Work Order", self.work_order)
			self.from_warehouse=parent_doc.virtual_warehouse
			self.to_warehouse=parent_doc.source_warehouse
			parent_doc = frappe.get_doc("Work Order", self.work_order)
			for row in parent_doc.get("required_items"):
				if(int(row.qsed_quantity)!=int(row.required_qty)):
					quantity=float(row.required_qty)-float(row.qsed_quantity)
					actual_quant=self.get_act_quant(self.from_warehouse,row.item_code)
					...
					if(quantity!=0):
						self.append(
								"items",
								{
									...
									"basic_rate":int(row.rate),
									"amount":int(row.rate)*quantity,
									"qty":quantity,
									"actual_qty":actual_quant
								}
						)
		
	def get_act_quant(self,warehouse,item):
		required=frappe.get_all("Bin",filters={'item_code':str(item),"warehouse":str(warehouse)},
												fields=["actual_qty"])
		actual=0
		for i in required:
			actual=i.actual_qty
			...
			if self.stock_entry_type=="Material Transfer for Virtual":
				...
				for row in parent_doc.get("required_items"):
						if(row.item_code==i.item_code):
							row.qsed_quantity=row.qsed_quantity+i.qty
				parent_doc.save()
    
	#use where On_cancel event
	def change_used(self):
		for i in self.get("items"):
			if self.stock_entry_type=="Material Transfer for Virtual":
				parent_doc = frappe.get_doc("Work Order", self.work_order)
				for row in parent_doc.get("required_items"):
	...
	def validate_warehouse(self):
		...
		for d in self.get("items"):
			...
			if cstr(d.s_warehouse) == cstr(d.t_warehouse) and self.purpose not in [
				...
				frappe.msgprint(_("Source and target warehouse cannot be same for row {0}").format(d.idx))
